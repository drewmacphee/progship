...
pub fn generate_crew(
    ...
    let room_data: Vec<(u32, f32, f32)> = rooms
        .iter()
        .enumerate()
        .filter_map(|(idx, &entity)| {
            world
                .get::<&crate::components::Room>(entity)
                .ok()
                .map(|room| (idx as u32, room.width(), room.depth()))
        })
        .collect();
    ...
    for i in 0..count {
        ...
        let crew_data = Crew::new(department, rank, shift).with_station(room_id);
...
mod tests {
    ...
    fn test_generate_crew() {
        ...
        let rooms: Vec<Entity> = (0..5).map(|_| world.spawn(())).collect();
