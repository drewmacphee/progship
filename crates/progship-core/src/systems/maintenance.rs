...
use crate::components::{
    ShipSystem, MaintenanceTask, Crew, Department, Activity, ActivityType, Position
};
...
impl MaintenanceQueue {
    ...
    pub fn add_task(&mut self, task: MaintenanceTask) {
        if !self.tasks.iter().any(|t| t.system_entity_id == task.system_entity_id) {
...
pub fn generate_maintenance_tasks(
    world: &World,
...
pub fn assign_maintenance_crew(
    world: &World,
    queue: &mut MaintenanceQueue,
) {
    ...
    for (entity, (crew, activity)) in world.query::<(&Crew, Option<&Activity>)>().iter() {
        ...
        let is_busy = activity.map(|a| a.activity_type == ActivityType::Maintenance).unwrap_or(false);
        if !is_busy {
...
mod tests {
    use super::*;
    use crate::components::{SystemType, Rank, Shift};
