    ...
    if distance < 0.1 || step >= distance {
        ...
        if !movement.path.is_empty() && movement.path_index < movement.path.len() - 1 {
            ...
            let new_movement = Movement {
                ...
                next_door_position: movement
                    .entry_door_positions
                    .get(movement.path_index + 2)
                    .copied(),
...
pub fn find_path(world: &World, from_room_id: u32, to_room_id: u32) -> Option<Vec<u32>> {
...
mod tests {
    ...
    fn test_multi_room_path() {
        ...
        let entity = world.spawn((Position::new(0.0, 0.0, 0), movement));
