...
use crate::components::{ShipSystem, SystemStatus, ResourceFlow, ResourceType, ResourceStorage};
...
impl ShipResources {
    ...
    pub fn level(&self, resource: ResourceType) -> f32 {
        ...
        if cap > 0.0 { current / cap } else { 0.0 }
    }
...
fn apply_resource_changes(resources: &mut ShipResources, _delta_hours: f32) {
    ...
    resources.storage.power = (resources.storage.power + net_power)
        .clamp(0.0, resources.capacity.power);
    
    // Water
    let net_water = resources.production.water - resources.consumption.water;
    resources.storage.water = (resources.storage.water + net_water)
        .clamp(0.0, resources.capacity.water);
    
    // Oxygen
    let net_oxygen = resources.production.oxygen - resources.consumption.oxygen;
    resources.storage.oxygen = (resources.storage.oxygen + net_oxygen)
        .clamp(0.0, resources.capacity.oxygen);
    
    // Food
    let net_food = resources.production.food - resources.consumption.food;
    resources.storage.food = (resources.storage.food + net_food)
        .clamp(0.0, resources.capacity.food);
    
    // Fuel (consumed by propulsion)
    let net_fuel = resources.production.fuel - resources.consumption.fuel;
    resources.storage.fuel = (resources.storage.fuel + net_fuel)
        .clamp(0.0, resources.capacity.fuel);
}
...
pub fn perform_maintenance(world: &mut World, system_entity: hecs::Entity, repair_amount: f32) -> bool {
