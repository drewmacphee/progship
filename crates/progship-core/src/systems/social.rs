...
use crate::components::{
    Activity, ActivityType, Conversation, ConversationState, ConversationTopic, Crew,
    InConversation, Name, Needs, Passenger, Person, Personality, Position, Relationship, Tone,
};
use hecs::World;
use rand::Rng;
...
impl RelationshipGraph {
    ...
    pub fn get_or_create(&mut self, person_a: u32, person_b: u32) -> &mut Relationship {
        ...
        if let Some(idx) = self
            .relationships
            .iter()
            .position(|r| r.person_a_id == a && r.person_b_id == b)
        {
    ...
    pub fn get(&self, person_a: u32, person_b: u32) -> Option<&Relationship> {
        ...
        self.relationships
            .iter()
            .find(|r| r.person_a_id == a && r.person_b_id == b)
    }
}
...
impl ConversationManager {
    ...
    pub fn cleanup(&mut self) -> Vec<(u32, Vec<u32>)> {
        let ended: Vec<(u32, Vec<u32>)> = self
            .conversations
            .iter()
            ...
            .collect();
        self.conversations
            .retain(|(_, c)| c.state != ConversationState::Ended);
        ended
    }
    ...
    pub fn active_count(&self) -> usize {
        self.conversations
            .iter()
            .filter(|(_, c)| c.state == ConversationState::Active)
            .count()
    }
}

/// Find people who are close enough to talk
pub fn find_nearby_pairs(world: &World, room_entities: &[hecs::Entity]) -> Vec<(u32, u32, u32)> {
    // (person_a_idx, person_b_idx, room_id)
    let mut pairs = Vec::new();

    // Group people by room
    let mut people_by_room: std::collections::HashMap<u32, Vec<(u32, hecs::Entity)>> =
...
mod tests {
    ...
    fn test_conversation_manager() {
        ...
        let id = manager.start_conversation(vec![1, 2], ConversationTopic::Greeting, 0.0);
